...
set _DeploySetParametersFile=%RootPath%Acom1.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Acom1.zip" (
echo "%RootPath%Acom1.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Acom1.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='AcomSite\\App_Data$'
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Acom1.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='AcomSite\\App_Data$' -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%Acom1.deploy-readme.txt" (
...
start notepad "%RootPath%Acom1.deploy-readme.txt"
)
echo =========================================================
goto :eof
